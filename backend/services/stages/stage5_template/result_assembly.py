...
def _step_5_6_pipeline_result(
    ...
    layout_types: list[LayoutTypeInfo] = [
        LayoutTypeInfo.model_validate(lt) if isinstance(lt, dict) else lt for lt in raw_layout_types
    ]
    ...
    first_layout_id = first_layout.id if first_layout else None
    ...
    layout_order = [lt.id for lt in layout_types if lt.id]
    ...
    result_json: dict[str, Any] = {
        ...
        "ambiguous_fields": [
            m
            for m in context.get("field_mappings", [])
            if (m.is_ambiguous if hasattr(m, "is_ambiguous") else m.get("is_ambiguous"))
        ],
