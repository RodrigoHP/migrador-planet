...
function inicializarCamposParaBindings() {
  ...
  const cPB = data.ConfirmacaoPagamentoBeneficio = data.ConfirmacaoPagamentoBeneficio || {};

  cPB.NomeBeneficiario ??= '';
  cPB.CPFBeneficiario ??= '';
  cPB.EmailBeneficiario ??= '';
  cPB.DocumentoParceiro ??= '';
  cPB.Logradouro ??= '';
  cPB.Numero ??= '';
  cPB.Bairro ??= '';
  cPB.Cep ??= '';
  ...
  cPB.Coberturas ??= {};
  cPB.Coberturas.Cobertura = garantirArray(
    cPB.Coberturas.Cobertura
  );

  cPB.Coberturas.Cobertura.forEach(cb => {
    cb.Inscricao ??= "";
    cb.Descricao ??= "";
    cb.Valor ??= "";
  });
  ...
  cPB.ICoberturas ??= {};
  cPB.ICoberturas.Cobertura = garantirArray(
    cPB.ICoberturas.Cobertura
  );

  cPB.ICoberturas.Cobertura.forEach(cb => {
    cb.Apolice ??= "";
  ...
  cPB.IACoberturas ??= {};
  cPB.IACoberturas.Cobertura = garantirArray(
    cPB.IACoberturas.Cobertura
  );

  cPB.IACoberturas.Cobertura.forEach(cb => {
...
function fatorGeradorPadrao(data) {
  ...
  let regra3 = data.ConfirmacaoPagamentoBeneficio.ICoberturas.Cobertura.some(cobertura => cobertura.Descricao === 'INCAPACIDADE FÍSICA TOTAL E TEMPORÁRIA');
...
function fatorGeradorI(data) {
  ...
  let regra2 = data.ConfirmacaoPagamentoBeneficio.ICoberturas.Cobertura.some(cobertura => cobertura.Descricao !== 'INCAPACIDADE FÍSICA TOTAL E TEMPORÁRIA');
...
function verificarSeEhProrrogacao() {
  if (data.ConfirmacaoPagamentoBeneficio.NumeroSequencial == 0)
    return false;
  else
    return true;
}
...
function iniciarRenderizacaoSequencialSincrono() {
  ...
  const pagamento = data.ConfirmacaoPagamentoBeneficio;

  pagamento.Endereco = dados;
  pagamento.DataFormatada = gerarDataFormatada();

  if (verificarSeEhProrrogacao()) {
    pagamento.NumeroProcesso = `${pagamento.NumeroProcesso}.${pagamento.NumeroSequencial}`;
    pagamento.ICoberturas.Cobertura.forEach(cb => { cb.Franquia = 0; });
  }
