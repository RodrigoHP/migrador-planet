...
function ajustarTipos() {
    ...
    data.Simulacao.ExibirValorContribuicao = getExibirColunaContribuicao(data.Simulacao.Produtos);
    ...
    for (var i = 0; i < data.Simulacao.Produtos.length; i++) {
        ...
        produto.ExibirReserva = verificarAlgumItemComPropriedadeTrue('PermiteResgateBAC', produto.TabelaResgate);
        produto.ExibirProlongamento = verificarAlgumItemComPropriedadeTrue('PermiteProlongamento', produto.TabelaResgate);
        produto.PossuiTabelaResgate = produto.ExibirReserva || produto.ExibirProlongamento;
...
function verificarAlgumItemComPropriedadeTrue(nomePropriedade, lista) {
    return lista.some(function(item) {
        return item.hasOwnProperty(nomePropriedade) && (item[nomePropriedade] === true || item[nomePropriedade] === "true");
    });
}
...
function prepararPaginasDadosDoPlano() {
    ...
    // Propriedades customizadas; que não existem no JSON de dados.
    data.Simulacao.ExibirIdade = data.Simulacao.IdadeProponente > 0;
    data.Simulacao.ExibirRendaMensal = data.Simulacao.RendaMensal > 0;
    ...
    for (var i = 0; i < data.Simulacao.Produtos.length; i++) {
        ...
        for (var j = 0; j < produto.Coberturas.length; j++) {
            ...
            var pageItem = { 
                ...
                "ExibirContribuicao": getExibirColunaContribuicao(data.Simulacao.Produtos),
                "ValorBeneficioZerado": cobertura.Beneficio == 0,
                "ValorContribuicaoZerado": cobertura.Contribuicao == 0,
            };
    ...
    for (var i = 0; i < data.Simulacao.Produtos.length; i++) {
        ...
        if (produto.TabelaResgate != null) {
            ...
            for (var element = 0; element < maxPerPage; element++) {
                TabelaResgate[element].ExibirReserva = produto.ExibirReserva;
                TabelaResgate[element].ExibirProlongamento = produto.ExibirProlongamento;
...
function getExibirColunaContribuicao(produtos) {
    return produtos.some(function(produto) {
        return produto.Coberturas.some(function(cobertura) {
            return cobertura.Contribuicao > 0;
        });
    });
}