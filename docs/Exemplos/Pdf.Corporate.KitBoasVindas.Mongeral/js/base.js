...
// Constantes para horários de vigência
const HORARIO_INICIO_VIGENCIA = '00h';
const HORARIO_FIM_VIGENCIA = '23h59';
...
function criarTextoProcessoSUSEP() {
  data.KIT_BOAS_VINDAS.TextoProcessoSUSEP = ko.computed(() => {
    ...
    if (planoPrev && planoPrev.PROC_SUSEP) {
      itens.push(planoPrev.NOME_PLANO_PREV + ': ' + planoPrev.PROC_SUSEP);
    }
    ...
    listaPlanosRisco.forEach(p => {
      // Só inclui se tiver processo SUSEP (assistências não têm processo próprio)
      if (p.PROC_SUSEP && p.PROC_SUSEP.trim() !== '') {
        itens.push(p.NOME_COBERTURA + ': ' + p.PROC_SUSEP);
      }
    });
...
function montarTextosCarencia() {
  ...
  // Também inclui os textos de CAUSA, se existir
  ...
  // Sempre adiciona o texto fixo de suicídio (ignora CARENCIA_SUICIDIO dos dados)
  const textoSuicidio = 'Para os casos de suicídio ou sua tentativa, haverá carência de 24 meses nos termos do Art. 120 - Lei 15.040.';
  textos.push(textoSuicidio);

  data.KIT_BOAS_VINDAS.TEXTOS_CARENCIA = textos;

  // Sempre exibe o bloco de carência
  data.KIT_BOAS_VINDAS.TEM_CARENCIA = true;
...
function montarFranquia() {
  ...
  // Só exibe o bloco de franquia se houver pelo menos um item (desconsiderando apenas o texto introdutório)
  // Se houver apenas 1 item, é só o texto genérico, então não exibe
  data.KIT_BOAS_VINDAS.TEXTOS_FRANQUIA = textosFranquia.length > 1 ? textosFranquia : [];
}

function montarGarantiasSeguro() {
  const produtos = data.KIT_BOAS_VINDAS?.DADOS_RISCO?.DADOS_PRODUTOS;

  if (!produtos || !Array.isArray(produtos) || produtos.length === 0) {
    data.KIT_BOAS_VINDAS.GARANTIAS_SEGURO = [];
    return;
  }

  // Mapeia os produtos para o formato de garantias (PRODUTO_ID - NOME: DESCRICAO)
  const garantias = produtos.map(produto => {
    const produtoId = produto.PRODUTO_ID || '';
    const nome = produto.NOME || '';
    const descricao = produto.DESCRICAO || '';

    return {
      NOME_COBERTURA: `${produtoId} - ${nome}`,
      DESCRICAO: descricao
    };
  });

  data.KIT_BOAS_VINDAS.GARANTIAS_SEGURO = garantias;
}
...
function ajustarPeriodoVigenciaPlanos() {
  ...
  listaPlanos.forEach(plano => {
    ...
    if (ini && fim) {
      plano.DATA_INI_VIGENCIA = `${ini} ${HORARIO_INICIO_VIGENCIA} \naté ${fim} ${HORARIO_FIM_VIGENCIA}`;
    } else if (ini) {
      plano.DATA_INI_VIGENCIA = `${ini} ${HORARIO_INICIO_VIGENCIA}`;
    }
...
function formatarCpfCnpj(documento) {
  ...
  let semFormatacao = documento.toString().replace(/[.\-\/\s]/g, '');

  // Verifica se é apenas numérico
  const apenasNumerico = /^\d+$/.test(semFormatacao);

  if (apenasNumerico) {
    // CPF: completa com zero à esquerda se tiver 10 dígitos
    if (semFormatacao.length === 10) {
      semFormatacao = "0" + semFormatacao;
    }

    // CNPJ: completa com zero à esquerda se tiver 13 dígitos
    if (semFormatacao.length === 13) {
      semFormatacao = "0" + semFormatacao;
    }

    // Formata CPF (11 dígitos)
    if (semFormatacao.length === 11) {
      return semFormatacao.replace(
        /(\d{3})(\d{3})(\d{3})(\d{2})/,
        "$1.$2.$3-$4"
      );
    }
    ...
    // CNPJ alfanumérico - verifica se tem apenas letras e números
    const apenasAlfanumerico = /^[a-zA-Z0-9]+$/.test(semFormatacao);

    if (!apenasAlfanumerico) {
      // Contém caracteres especiais, retorna sem formatar
      return documento;
    }

    // CNPJ alfanumérico: formata SOMENTE se tiver exatamente 14 caracteres
    // NÃO completa com zeros à esquerda
    if (semFormatacao.length === 14) {
      return semFormatacao.replace(
        /^(.{2})(.{3})(.{3})(.{4})(.{2})$/,
        "$1.$2.$3/$4-$5"
  ...
  // Se não conseguir formatar, retorna o original sem alterações
  return documento;
}
...
function identificarTipoDocumento(documento) {
  ...
  // Remove caracteres não alfanuméricos para identificar o tamanho
  const somenteAlfanumerico = documento.toString().replace(/[^a-zA-Z0-9]/g, '');

  // CNPJ tem 14 caracteres, CPF tem 11
  if (somenteAlfanumerico.length >= 13) {
    return "CNPJ";
  } else if (somenteAlfanumerico.length >= 10) {
    return "CPF";
  }
...
function aplicarFormatacaoTodosDocumentos() {
  // CPF do Segurado
  const segurado = data?.KIT_BOAS_VINDAS?.DADOS_SEGURADO;
  if (segurado?.CPF) {
    segurado.CPF = formatarCpfCnpj(segurado.CPF);
  }

  // CPF/CNPJ do Corretor
  const corretor = data?.KIT_BOAS_VINDAS?.DADOS_RISCO?.CORRETOR;
  if (corretor?.DOCUMENTO) {
    corretor.DOCUMENTO = formatarCpfCnpj(corretor.DOCUMENTO);
  }

  // CNPJ do Estipulante
  const apolice = data?.KIT_BOAS_VINDAS?.DADOS_RISCO?.APOLICE;
  if (apolice?.CNPJ_ESTIPULANTE) {
    apolice.CNPJ_ESTIPULANTE = formatarCpfCnpj(apolice.CNPJ_ESTIPULANTE);
  }

  // CPF/CNPJ do Titular da Conta de Pagamento
  const dadosPagamento = data?.KIT_BOAS_VINDAS?.DADOS_PAGAMENTO;
  if (dadosPagamento?.CPFCNPJ_TITULAR_CONTA) {
    dadosPagamento.CPFCNPJ_TITULAR_CONTA = formatarCpfCnpj(dadosPagamento.CPFCNPJ_TITULAR_CONTA);
  }
}
...
function iniciarRenderizacaoSequencialSincrono() {
  ...
  aplicarFormatacaoTodosDocumentos()
  ...
  const documentoDadosTitularPagamento = identificarTipoDocumento(data?.KIT_BOAS_VINDAS?.DADOS_PAGAMENTO?.CPFCNPJ_TITULAR_CONTA);
  ...
  if (existeRepeticaoRenderizada('tabela-row-beneficiarios')) {
    ...
    mostrarElementoEReaplicarBindEReposicionar('.texto-segurado-beneficiario')
  } else {
    mostrarElementoEReaplicarBindEReposicionar('.texto-beneficiarios-herdeiros')
    mostrarElementoEReaplicarBindEReposicionar('.texto-segurado-herdeiro')
  }
  ...
  // Só exibe INFORMAÇÕES ADICIONAIS se tiver DADOS_RISCO com planos
  if (existeRepeticaoRenderizada('tabela-row-dados-planos-morte-ou-invalidez')) {
  ...
  if (existeRepeticaoRenderizada('item-garantia')) {
    mostrarElementoEReaplicarBindEReposicionar('.bloco-garantias-seguro')
  }

  // Só exibe RISCOS EXCLUÍDOS e DESPESAS COM SALVAMENTO se tiver DADOS_RISCO com planos
  if (existeRepeticaoRenderizada('tabela-row-dados-planos-morte-ou-invalidez')) {
    mostrarElementoEReaplicarBindEReposicionar('.bloco-riscos-excluidos')
  ...
  // Só exibe CARÊNCIA se tiver DADOS_RISCO com planos
  if (existeRepeticaoRenderizada('tabela-row-dados-planos-morte-ou-invalidez')) {
  ...
  // Só exibe LOCAIS COBERTOS se tiver DADOS_RISCO com planos
  if (existeRepeticaoRenderizada('tabela-row-dados-planos-morte-ou-invalidez')) {
    mostrarElementoEReaplicarBindEReposicionar('.texto-locais-cobertos-group')
  }
  ...
  // Só exibe textos sobre condições gerais de seguro se tiver DADOS_RISCO com planos
  if (existeRepeticaoRenderizada('tabela-row-dados-planos-morte-ou-invalidez')) {
    mostrarElementoEReaplicarBind('.texto-final-condicoes-seguro')
  }
