...
function inicializarCamposParaBindings() {
    ...
    const pc = data.PagamentoConta = data.PagamentoConta || {};

    pc.NomeBeneficiario ??= '';
    pc.CPFBeneficiario ??= '';
    pc.EmailBeneficiario ??= '';
    pc.DocumentoParceiro ??= '';
    pc.Logradouro ??= '';
    pc.Numero ??= '';
    pc.Bairro ??= '';
    pc.Cep ??= '';
    ...
    pc.Coberturas ??= {};
    pc.Coberturas.Cobertura = garantirArray(
        pc.Coberturas.Cobertura
    );

    pc.Coberturas.Cobertura.forEach(cb => {
        cb.Inscricao ??= "";
        cb.Descricao ??= "";
        cb.Valor ??= "";
    });
    ...
    function formatarData(data) {
        ...
        const match = data.match(regexYMD);
        if (match) {
    ...
    pc.ICoberturas ??= {};
    pc.ICoberturas.Cobertura = garantirArray(
        pc.ICoberturas.Cobertura
    );

    pc.ICoberturas.Cobertura.forEach(cb => {
        cb.Apolice ??= "";
        cb.DataInicio = formatarData(cb.DataInicio) || "";
        cb.DataFinal = formatarData(cb.DataFinal) || "";
        cb.DiariasSolicitadas ??= "";
        cb.Franquia ??= "";
    ...
    pc.IACoberturas ??= {};
    pc.IACoberturas.Cobertura = garantirArray(
        pc.IACoberturas.Cobertura
    );

    pc.IACoberturas.Cobertura.forEach(cb => {
...
function fatorGeradorPadrao(data) {
    const sigla = data.PagamentoConta.Sigla ?? "";

    const coberturas = data.PagamentoConta.IACoberturas.Cobertura;
    let qtdCoberturas = 0;
    ...
    const bloqueadas = ["ITA", "ITD", "IH", "IHA", "IA"];

    const regra1 = !bloqueadas.includes(sigla);

    const regra2 = sigla === "IA" && qtdCoberturas === 0;

    let regra3 = data.PagamentoConta.ICoberturas.Cobertura.some(cobertura => cobertura.Descricao === 'INCAPACIDADE FÍSICA TOTAL E TEMPORÁRIA');
...
function fatorGeradorI(data) {
    ...
    const regra1 = ["ITA", "ITD", "IH", "IHA"].includes(sigla);

    let regra2 = data.PagamentoConta.ICoberturas.Cobertura.some(cobertura => cobertura.Descricao !== 'INCAPACIDADE FÍSICA TOTAL E TEMPORÁRIA');
...
function formatarDadosBeneficiario(carta = data.PagamentoConta) {
...
function gerarDataFormatada(dataString = data.PagamentoConta.DataStatus) {
...
function formatarEstruturaCoberturasIA() {
    data.PrimeiraInscricao = data.PagamentoConta.Coberturas.Cobertura[0].Inscricao;
    data.ValorBeneficio = data.PagamentoConta.ValorBeneficio || '';
}

function verificarSeEhProrrogacao() {
    if (data.PagamentoConta.NumeroSequencial == 0)
        return false;
    else
        return true
}
...
function iniciarRenderizacaoSequencialSincrono() {
    ...
    const pc = data.PagamentoConta;

    pc.Endereco = dados;
    pc.DataFormatada = gerarDataFormatada();

    if (verificarSeEhProrrogacao()) {
        pc.NumeroProcesso = `${pc.NumeroProcesso}.${pc.NumeroSequencial}`;
        pc.ICoberturas.Cobertura.forEach(cb => { cb.Franquia = 0; });
    }
