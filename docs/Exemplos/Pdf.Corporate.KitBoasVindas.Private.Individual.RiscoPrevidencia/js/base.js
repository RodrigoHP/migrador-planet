...
function criarTextoProcessoSUSEP() {
  const dados = data.KIT_BOAS_VINDAS;

  // Texto somente para PREVIDÊNCIA
  dados.TextoSUSEPPrev = ko.computed(() => {
    const planoPrev = dados.DADOS_PREV?.PLANOS;
    if (planoPrev) {
      return `${planoPrev.NOME_PLANO_PREV}: ${planoPrev.PROC_SUSEP}; `;
    }
    return '';
  });

  // Texto somente para RISCO
  dados.TextoSUSEPRisco = ko.computed(() => {
    const planosRisco = dados.DADOS_RISCO?.PLANOS_COBERTURAS;
    const listaPlanosRisco = Array.isArray(planosRisco)
      ? planosRisco
      : (planosRisco ? [planosRisco] : []);

    return listaPlanosRisco
      .map(p => `${p.NOME_COBERTURA}: ${p.PROC_SUSEP}; `)
      .join('');
  });

  // Texto combinado (opcional)
  dados.TextoProcessoSUSEP = ko.computed(() => {
    return dados.TextoSUSEPPrev() + dados.TextoSUSEPRisco();
  });
}
...
function inicializarCamposParaBindings() {
  ...
  // DADOS_PRODUTOS
  r.DADOS_PRODUTOS = garantirArray(r.DADOS_PRODUTOS);
  r.DADOS_PRODUTOS.forEach(d => {
    d.PRODUTO_ID ??= '';
    d.NOME ??= '';
    d.DESCRICAO ??= '';
  });
...
function montarTextosCarencia() {
  ...
  // Meses
  if (franquia?.CARENCIA_DEFINIDA_MESES) {
    const meses = Array.isArray(franquia.CARENCIA_DEFINIDA_MESES)
      ? franquia.CARENCIA_DEFINIDA_MESES
      : [franquia.CARENCIA_DEFINIDA_MESES];
    meses.forEach(item => {
      if (item?.TEXTO_CARENCIA_DEFINIDA) {
        textos.push(item.TEXTO_CARENCIA_DEFINIDA.trim());
      }
    });
  }

  // Dias
  if (franquia?.CARENCIA_DEFINIDA_DIA) {
    const dias = Array.isArray(franquia.CARENCIA_DEFINIDA_DIA)
      ? franquia.CARENCIA_DEFINIDA_DIA
      : [franquia.CARENCIA_DEFINIDA_DIA];
    dias.forEach(item => {
      if (item?.TEXTO_CARENCIA_DEFINIDA) {
        textos.push(item.TEXTO_CARENCIA_DEFINIDA.trim());
  ...
  // Causa
  if (franquia?.CARENCIA_PROGRESSIVA_CAUSA?.TEXTO_CARENCIA_PROGRESSIVA) {
    textos.push(franquia.CARENCIA_PROGRESSIVA_CAUSA.TEXTO_CARENCIA_PROGRESSIVA.trim());
  }

  // Suicídio
  const textoFixoCarenciaSuicidio = "Para os casos de suicídio ou sua tentativa, haverá carência de 24 meses nos termos do Art. 120 - Lei 15.040.";
  textos.push(textoFixoCarenciaSuicidio);
...
function montarFranquia() {
  ...
  const listaFranquia = Array.isArray(franquia)
    ...
    const textos = listaFranquia
    .map(f => f?.TEXTO_FRANQUIA?.trim())
        .filter(Boolean);

    // Guarda o subtítulo (primeiro item)
    const subtituloFranquia = textos[0] || "";

    // Demais textos (já sem o primeiro)
    const textosFranquia = textos.slice(1);

    data.KIT_BOAS_VINDAS.SUBTITULO_FRANQUIA = subtituloFranquia;
...
function ajustarPeriodoVigenciaPlanos() {
    ...
    listaPlanos.forEach(plano => {
        const iniRaw = typeof plano.DATA_INI_VIGENCIA === 'string' ? plano.DATA_INI_VIGENCIA.trim() : '';
        const fimRaw = typeof plano.DATA_FIM_VIGENCIA === 'string' ? plano.DATA_FIM_VIGENCIA.trim() : '';

        if (!iniRaw) return;

        const ini = `${iniRaw} 00h`;
        const fim = fimRaw ? `até ${fimRaw} 23h59` : '';

        plano.DATA_INI_VIGENCIA = ini;
        plano.DATA_FIM_VIGENCIA = fim;
    });
}

function ajustarPeriodoInicialPrevidencia() {
    const plano = data.KIT_BOAS_VINDAS?.DADOS_PREV?.PLANOS;
    if (!plano) return;

    const iniRaw = typeof plano.DATA_INI_VIGENCIA === 'string'
        ? plano.DATA_INI_VIGENCIA.trim()
        : '';

    if (!iniRaw) return;

    plano.DATA_INI_VIGENCIA = `${iniRaw} 00h`;
}


function ajustarPeriodoApolices() {
    const apolices = data?.KIT_BOAS_VINDAS?.DADOS_RISCO?.APOLICE?.DADOS_APOLICES;

    if (!apolices) return;

    const lista = Array.isArray(apolices) ? apolices : [apolices];

    lista.forEach(item => {
        const emissaoRaw = typeof item.DATA_EMISSAO === 'string' ? item.DATA_EMISSAO.trim() : '';
        const fimRaw = typeof item.DATA_FIM_VIGENCIA === 'string' ? item.DATA_FIM_VIGENCIA.trim() : '';

        if (emissaoRaw) {
            item.DATA_EMISSAO = `${emissaoRaw} 00h`;
        }

        if (fimRaw) {
            item.DATA_FIM_VIGENCIA = `${fimRaw} 23h59`;
        }
    });
}

function ajustarTextoIndexador() {
    const plano = data?.KIT_BOAS_VINDAS?.DADOS_RISCO;
    if (!plano || !plano.INDEX_PLANO) return;

    plano.TEXTO_INDEXADOR = `O seu seguro será atualizado pelo ${plano.INDEX_PLANO}. Para mais informações, consulte as condições gerais.`;
}
...
function identificarEFormatarCpfCnpjTitular() {
    ...
    const resultado = formatarDocumento(dadosPagamento.CPFCNPJ_TITULAR_CONTA);

    dadosPagamento.CPFCNPJ_TITULAR_CONTA = resultado.valor;
    return resultado.tipo;
}

function formatarCpfCnpjCorrentista() {
    const corretor = data?.KIT_BOAS_VINDAS?.DADOS_RISCO?.CORRETOR;
    if (!corretor || !corretor.DOCUMENTO) return;

    const resultado = formatarDocumento(corretor.DOCUMENTO);

    corretor.DOCUMENTO = resultado.valor;
}

function formatarCnpjEstipulante() {
    const estipulante = data?.KIT_BOAS_VINDAS?.DADOS_RISCO?.APOLICE;
    if (!estipulante || !estipulante.CNPJ_ESTIPULANTE) return;

    const resultado = formatarDocumento(estipulante.CNPJ_ESTIPULANTE);

    estipulante.CNPJ_ESTIPULANTE = resultado.valor;
}

function formatarCpfSegurado() {
    const segurado = data?.KIT_BOAS_VINDAS?.DADOS_SEGURADO;
    if (!segurado || !segurado.CPF) return;

    const resultado = formatarDocumento(segurado.CPF);

    segurado.CPF = resultado.valor;
}

function formatarDocumento(doc) {
    if (!doc) return { tipo: "CPF", valor: "" };

    let clean = doc.replace(/[^A-Za-z0-9]/g, "").toUpperCase();
    const somenteNumeros = /^\d+$/;
    const cnpjAlfanumerico = /^[A-Z0-9]{14}$/;
    const contemLetra = /[A-Z]/;

    // ---------- CPF numérico (10 ou 11) ----------
    if (somenteNumeros.test(clean)) {
        if (clean.length === 10) clean = "0" + clean;

        if (clean.length === 11) {
            return {
                tipo: "CPF",
                valor: clean.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, "$1.$2.$3-$4")
            };
        }
    }

    // ---------- CNPJ numérico (13 ou 14) ----------
    if (somenteNumeros.test(clean)) {
        ...
        if (clean.length === 14) {
            return {
                tipo: "CNPJ",
                valor: clean.replace(/(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, "$1.$2.$3/$4-$5")
            };
        }
    }

    // ---------- CNPJ alfanumérico (14 caracteres) ----------
    if (cnpjAlfanumerico.test(clean)) {
        return {
            tipo: "CNPJ",
            valor: clean.replace(/(.{2})(.{3})(.{3})(.{4})(.{2})/, "$1.$2.$3/$4-$5")
        };
    }

    // ---------- Caso inválido ----------
    if (contemLetra.test(clean)) {
        return {
            tipo: "CNPJ",
            valor: clean
        };
...
function iniciarRenderizacaoSequencialSincrono() {
  const dadosRisco = data.KIT_BOAS_VINDAS?.DADOS_RISCO;

  inicializarCamposParaBindings()
  ajustarPeriodoVigenciaPlanos()
  ajustarPeriodoInicialPrevidencia()
  processarPlanosComPrazos()
  regionDateFromBR('Rio de Janeiro')
  montarBeneficiarios()
  formatarCpfCnpjCorrentista()
  formatarCnpjEstipulante()
  formatarCpfSegurado()
  ajustarPeriodoApolices()
  ajustarTextoIndexador()
  const ehRenovacao = data?.KIT_BOAS_VINDAS?.RENOVACAO === "SIM";
  const documentoDadosTitularPagamento = identificarEFormatarCpfCnpjTitular();
  ...
  const possuiPrevidencia = dadosPrev && planos && temValorUtil(planos);

  if (possuiPrevidencia) {
    mostrarElementoEReaplicarBind('.header-inicial-prev')
    if (temNomeSocial()) {
      mostrarElementoEReaplicarBindEReposicionar('.tabela-segurado-com-nome-civil-prev')
    }
    else {
      mostrarElementoEReaplicarBindEReposicionar('.tabela-segurado-prev')
    }
    ...
    if (!ehRenovacao) {
      if (exibeTabelaDadosBasicosPagamento) {
        mostrarElementoEReaplicarBindEReposicionar('.tabela-dados-pagamento-basicos-prev');
      } else if (documentoDadosTitularPagamento === 'CPF') {
        mostrarElementoEReaplicarBindEReposicionar('.tabela-dados-pagamento-com-cpf-prev');
      } else if (documentoDadosTitularPagamento === 'CNPJ') {
        mostrarElementoEReaplicarBindEReposicionar('.tabela-dados-pagamento-com-cnpj-prev');
      }
    }

    mostrarElementoEReaplicarBindEReposicionar('.texto-dados-susep-group-prev')

    if(!dadosRisco) {
      mostrarElementoEReaplicarBindEReposicionar('.texto-final-group-prev')
    }
  }
  ...
  if (dadosRisco) {
    ...
    mostrarElementoEReaplicarBind('.header-inicial')
    reposicionarElementoFixo('.header-inicial', '1.6in')
    ...
    if (existeRepeticaoRenderizada('tabela-row-beneficiarios')) {
      quebrarTabelaEntrePaginas('tabela-beneficiarios', 'tabela-header-beneficiarios', 'tabela-row-beneficiarios', 'texto-beneficiarios-footer')
    } else {
    ...
    mostrarElementoEReaplicarBindEReposicionar('.texto-garantia-seguro')
    ...
    mostrarElementoEReaplicarBindEReposicionar('.texto-riscos-excluidos');
    mostrarElementoEReaplicarBindEReposicionar('.texto-despesa-com-salvamento');

    mostrarElementosComValorEReaplicarBind('.texto-seguro');
    reposicionarElementoFixo('.texto-seguro')

    if (existeRepeticaoRenderizada('row-carencia')) {
    ...
    mostrarElementoEReaplicarBindEReposicionar('.texto-locais-coberto-seguro');
...
function regionDateFromBR(region) {
  let result = '';

  const dataEmissao = data.KIT_BOAS_VINDAS.DATA_EMISSAO
  if (dataEmissao.length > 7) {
    const firstSlash = dataEmissao.indexOf('/');
    const dia = dataEmissao.substring(0, firstSlash);

    let rest = dataEmissao.substring(firstSlash + 1);
    const secondSlash = rest.indexOf('/');
    const mes = parseInt(rest.substring(0, secondSlash), 10);

    const ano = rest.substring(secondSlash + 1);
    ...
    if (mes >= 1 && mes <= 12) {
      nomeMes = arrMonthNames[mes - 1];
    }

    result = `${region}, ${dia} de ${nomeMes} de ${ano}`;
  }

  data.KIT_BOAS_VINDAS.EMISSAO_FORMATADA = result ? result : '';
}


function quebrarTabelaEntrePaginas(seletorClasse, seletorHeader, seletorRow, seletorFooter = null, margemEspaco = '0.2in', margemQuebra = '1.6in') {
...
function criarNovaPagina(temFundo = true) {
  if (temFundo) {
    ...
    backgroundImg.src = './img/private_intro.png';
...
function reposicionarElementoFixo(seletor, margemEspacoPadrao = '0.2in', margemQuebraPadrao = '1.6in') {
